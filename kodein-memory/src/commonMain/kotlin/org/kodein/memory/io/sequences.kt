...
public fun Readable.asSequence(): Sequence<Byte> = sequence { while (valid()) yield(readByte()) }
public fun Readable.asCharSequence(): Sequence<Char> = sequence { while (valid()) yield(readChar()) }
...
public fun Readable.asUSequence(): Sequence<UByte> = sequence { while (valid()) yield(readUByte()) }
@ExperimentalUnsignedTypes
